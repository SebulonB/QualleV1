...
void led_visuals_init(void){
	...
	ws2812b_get_buffer(&digLED);	
	
	// Set output channel/pin, GPIO_PIN_0 = 0, for GPIO_PIN_5 = 5 - this has to correspond to WS2812B_PINS
	digLED.item[0].channel = 0;
	// Your RGB framebuffer
	digLED.item[0].frameBufferPointer = frameBuffer;
	// RAW size of framebuffer
	digLED.item[0].frameBufferSize = sizeof(frameBuffer);


	// If you need more parallel LED strips, increase the WS2812_BUFFER_COUNT value
	digLED.item[1].channel = 3;
	digLED.item[1].frameBufferPointer = frameBuffer2;
	digLED.item[1].frameBufferSize = sizeof(frameBuffer2);
